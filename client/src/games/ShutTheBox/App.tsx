...
export type ShutTheBoxProps = {
  appDependencies: ShutTheBoxDependencies;
};
export type ShutTheBoxDependencies = {
  gameAnalysisService: GameAnalysisService;
};
export function ShutTheBox(props: ShutTheBoxProps) {
  const [gameState, setGameState] = React.useState<GameState>(
    createGameState()
  );

  const toggleTileOpen = React.useCallback(
    (tile: Tile) => {
  ...
  const setDie1 = React.useCallback(
    (pips: D6) => {
      setGameState({
        ...gameState,
        d1: pips,
  ...
  const setDie2 = React.useCallback(
    (pips: D6) => {
      setGameState({
        ...gameState,
        d2: pips,
