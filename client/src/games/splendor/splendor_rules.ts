...
export function initSplendorState(): SplendorState {
  const tier1Shuffled = shuffle(tier1Cards);
  const tier2Shuffled = shuffle(tier2Cards);
  const tier3Shuffled = shuffle(tier3Cards);

  const [tier1Deck, tier1Board] = drawN(tier1Shuffled, 4);
  const [tier2Deck, tier2Board] = drawN(tier2Shuffled, 4);
  const [tier3Deck, tier3Board] = drawN(tier3Shuffled, 4);

  const [_unusedTiles, tiles] = drawN(shuffle(locationTiles()), 4);

  return {
    board: [tier1Board, tier2Board, tier3Board],
    cardStacks: [tier1Deck, tier2Deck, tier3Deck],
    gems: {
      purple: 7,
      red: 7,
      orange: 7,
...
function locationTiles(): Array<LocationTile> {
  return [
    triskelionTile({ purple: 4, orange: 4 }),
    hellsKitchenNyc({ orange: 4, yellow: 4 }),
    knowhere({ red: 3, blue: 3, yellow: 3 }),
    atlantis({ purple: 3, red: 3, blue: 3 }),
    wakanda({ red: 4, blue: 4 }),
    asgard({ purple: 3, orange: 3, yellow: 3 }),
    avengersTowerNyc({ blue: 4, yellow: 4 }),
    attilan({ purple: 4, red: 4 }),
  ];
}
